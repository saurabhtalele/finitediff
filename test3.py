# -*- coding: utf-8 -*-
"""
Created on Fri Apr 30 04:44:18 2021
...
from pywebio.output import put_text, put_html, put_image, put_widget
...
# total number of points in cosideration
    ...
    put_text('for error order result if its greter than this please relod web and try max limit integer number is equal or less than (in central diff scheme) in between ',
             int(-1*(dd+err-1)/2), 'and ', int((dd+err-1)/2))
    put_html('<hr>')
...
# take limits from -3 to 3 for example ,so points in cosiderations are -3,-2,-1,0,1,2,3 total n is 7
    ...
    put_text(
        '-3 to 3 for example ,so points in cosiderations are -3,-2,-1,0,1,2,3 so min_limit as -3')
...
# max limit
    put_text(
        '-3 to 3 for example ,so points in cosiderations are -3,-2,-1,0,1,2,3 so max_limit as 3')
